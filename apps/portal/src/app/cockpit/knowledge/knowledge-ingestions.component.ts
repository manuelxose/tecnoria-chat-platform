    ...
    <div class="ck-content">
      ...
      <div class="ck-grid-sidebar">
              ...
              <table class="ck-table">
                ...
                <tbody>
                  @for (job of filtered; track job.id) {
                    <tr>
                      <td class="ck-table__cell--mono" style="font-size: 0.75rem;">{{ job.id.slice(0, 8) }}…</td>
                      <td>
                        <div class="ck-table__cell--strong">{{ job.projectKey }}</div>
                        <div style="font-size: 0.75rem; color: var(--ck-text-muted);">{{ job.sourceKey }}</div>
                      </td>
                      <td>
                      ...
                      <td style="color: var(--ck-text-muted); font-size: 0.78rem;">{{ job.requestedBy }}</td>
        ...
        <div class="ck-card">
          ...
          @if (queueSuccess) {
            <div class="ck-alert ck-alert--success" style="margin-bottom: 12px;">Job queued successfully.</div>
          }
          <div class="ck-form-stack">
            ...
            <button class="ck-btn ck-btn--primary" style="width: 100%;" (click)="queue()" [disabled]="queuing">
          ...
          <div style="display: grid; gap: 6px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="font-size: 0.8rem; color: var(--ck-text-soft);">Running</span>
              <span class="ck-badge ck-badge--info">{{ countByStatus('running') }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="font-size: 0.8rem; color: var(--ck-text-soft);">Queued</span>
              <span class="ck-badge ck-badge--warning">{{ countByStatus('queued') }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="font-size: 0.8rem; color: var(--ck-text-soft);">Done</span>
              <span class="ck-badge ck-badge--success">{{ countByStatus('done') }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="font-size: 0.8rem; color: var(--ck-text-soft);">Failed</span>
